...
import com.fit.se.common.exception.errors.AccountLockedException;
import com.fit.se.common.exception.errors.AccountProvisioningException;
import com.fit.se.common.exception.errors.InvalidCredentialsException;
...
public class SignInUseCase {
        ...
        if (!jtis.isEmpty()) {
            List<String> matchedJtis = jtis.stream()
                    .filter(jti -> {
                        ...
                        return Objects.equals(currentDevice, storedDevice);
                    })
                    .toList();
    ...
    private Account authenticateByPassword(String phone, String password) {
        ...
        Account storedAccount = accountRepo.findByPhone(normalizedPhone)
        ...
        if (!storedAccount.isProfileProvisioned()) {
